...
{
    public class ApiResourceRepository : EfCoreRepository<IAuthserviceDbContext, ApiResource, Guid>, IApiResourceRepository
    {
        public ApiResourceRepository(IDbContextProvider<IAuthserviceDbContext> dbContextProvider) : base(dbContextProvider)
        {
            ...
            CancellationToken cancellationToken = default)
        {
            return await (await GetDbSetAsync())
                .IncludeDetails(includeDetails)
                .WhereIf(!filter.IsNullOrWhiteSpace(), x => x.Name.Contains(filter!) || x.Description.Contains(filter!) || x.DisplayName.Contains(filter!))
        ...
        public virtual async Task<long> GetCountAsync(string? filter = null, CancellationToken cancellationToken = default)
        {
            return await (await GetDbSetAsync())
                .WhereIf(!filter.IsNullOrWhiteSpace(),
                    x => x.Name.Contains(filter!) || x.Description.Contains(filter!) || x.DisplayName.Contains(filter!))
