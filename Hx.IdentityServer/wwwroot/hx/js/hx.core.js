    ...
    var _hxCore = {
        ...
        */
        ...
        ajaxSuccess(r, success, failure) {
            ...
            data = result;
            if (result) {
                const successKey = hxCore.defaults.successkey,
                    dataKey = hxCore.defaults.datakey
                if (result.hasOwnProperty(successKey)) {
                    succeed = result[successKey];
            ...
            if (succeed) {
                if (hxCore.isFunction(success)) {
                    success.call(this, data, result, r);
                }
            } else {
                const messageKey = hxCore.defaults.messagekey;
                const msg = result[messageKey] || '';
                if (hxCore.isFunction(failure)) {
                    failure.call(this, msg, result, r);
        ...
        ajaxError(r, failure) {
            ...
            switch (r.status) {
                case 403:
                    msg = hxCore.defaults.notice['code' + r.status];
                    break;
                case 404:
                    msg = hxCore.defaults.notice['code' + r.status];
                    break;
