...
const handleQuery = async () => {
	state.loading = true;
	var res = await getAPI(SysNoticeApi).getReceivedPage(state.queryParams.title, state.queryParams.type, state.tableParams.page, state.tableParams.pageSize);
...
const viewDetail = async (row: any) => {
	...
	await getAPI(SysNoticeApi).setSysNoticeRead({ id: row.sysNotice.id });
};
